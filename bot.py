...
CONTRACT_ADDRESS = Web3.to_checksum_address("0x983f280af2d271244d08808ea023549278fdea9d")
...
def mint_nft(w3, contract, private_key, key_index, total_keys):
    try:
        ...
        min_balance = w3.to_wei(0.1, "ether") + 200000 * w3.to_wei(10, "gwei")  
        if balance < min_balance:
            print(f"{Fore.YELLOW}Insufficient balance for {public_key}: {w3.from_wei(balance, 'ether')} PHRS, required: {w3.from_wei(min_balance, 'ether')} PHRS")
            return False
        ...
        price_per_token = w3.to_wei(0.1, "ether")  
        allowlist_proof = (
            [],
            0,
            0,
        ...
        if receipt.status == 1:
            ...
            print(f"{Fore.GREEN}View transaction: https://atlantic.pharosscan.xyz/tx/{w3.to_hex(tx_hash)}")
            return True
        else:
...
def main():
    ...
    print(f"{Fore.MAGENTA}########## PHAROS_ATLANTIC Badge 2 Mint Bot##########")
